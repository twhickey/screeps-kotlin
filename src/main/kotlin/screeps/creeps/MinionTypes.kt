...
enum class MinionType(val bodyPartPriorities: Map<BodyPartConstant, BodyPartSpec>, val validStates: List<CreepState>) {
    ...
    ), listOf(CreepState.REPAIR, CreepState.REPAIR_WALLS, CreepState.BUILDING, CreepState.TRANSFERRING_ENERGY, CreepState.GETTING_ENERGY, CreepState.UPGRADING, CreepState.SCOUT)),
    HAULER(mapOf(
        WORK to BodyPartSpec(1, 5, 0.5),
        CARRY to BodyPartSpec(3, 10, 1.0),
        MOVE to BodyPartSpec(2, 5, 1.0)
    ), listOf(CreepState.GETTING_ENERGY, CreepState.TRANSFERRING_ENERGY, CreepState.BUILDING, CreepState.UPGRADING, CreepState.HAUL)),
    MINER(mapOf(
